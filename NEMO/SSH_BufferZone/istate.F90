MODULE istate
   ...
   USE dtassh          ! data SSH                        (dta_ssh routine)
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: istate.F90 5163 2015-03-24 12:21:17Z gm $
...
CONTAINS
   ...
   SUBROUTINE istate_init
      ...
      CALL dta_ssh_init                       ! Initialisation of SSH input data
      ...
      ELSE
        ...
        ELSEIF( cp_cfg == 'isomip' .OR. cp_cfg == 'isomip2') THEN
         ...
         ELSE                                    ! Initial T-S, U-V fields read in files
            ...
            ENDIF
            IF ( ln_ssh_init ) THEN              ! read 2D SSH data at nit000
              CALL dta_ssh( nit000, sshb )
              sshn(:,:) = sshb(:,:)
            ENDIF
         ENDIF
         !
